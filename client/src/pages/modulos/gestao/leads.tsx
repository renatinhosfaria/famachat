...
import { Lead, User, Cliente, LeadAutomationConfig as DBLeadAutomationConfig, ClienteStatus } from "@shared/schema";
        ...
        <ScrollArea className="flex-grow pr-4 mt-4">
          <Tabs defaultValue="users">
                    ...
                    .map((user: any) => (
                      <div key={user.id} className="flex items-center space-x-2 p-2 border rounded-md">
                        <Checkbox
                          ...
                          checked={localConfig.selectedUsers?.includes(user.id) || false}
                        ...
                        />
                        <label htmlFor={`user-automation-${user.id}`} className="text-sm font-medium leading-none">
                ...
                <div className="space-y-4 mt-6">
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto border rounded-md p-3">
                    {localConfig.selectedUsers.map((userId, index) => {
                      ...
                      return (
                        <div key={userId} className="flex items-center justify-between p-3 border rounded bg-gray-50">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Use as setas para reordenar os usuários. A distribuição seguirá esta ordem.
                  </p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="sla" className="space-y-4 mt-4">
              <div className="space-y-4">
                <h3 className="text-lg font-medium">SLA em Cascata</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  Configure o tempo de exclusividade por usuário e a ordem da fila de atendimento.
                </p>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Cada usuário terá este tempo para conseguir um agendamento antes do lead passar para o próximo.
                    </p>
                  </div>
...
const LeadsPage: React.FC = () => {
  ...
  const { data: automationConfigData } = useQuery<DBLeadAutomationConfig>({
  ...
  useEffect(() => {
    if (automationConfigData) {
      setAutomationConfig({
        ...
        selectedUsers: automationConfigData.rotationUsers ?? [],
      });
    }
  ...
  // Busca de leads da tabela sistema_leads
  const { data: leadsResponse, isLoading } = useQuery<{leads: Lead[], total: number}>({
  ...
  const handleSaveAutomationConfig = async (config: AutomationConfig) => {
    ...
    try {
      ...
      const apiData = {
        ...
        // Configurações de SLA em cascata
      ...
      const updatedConfig = await response.json() as DBLeadAutomationConfig;
      ...
      setAutomationConfig({
        ...
        selectedUsers: updatedConfig.rotationUsers ?? [],
      });
  ...
  return (
    <div className="container mx-auto py-6">
      <Helmet>
        <title>Leads | Fama Negócios Imobiliários</title>
      </Helmet>
