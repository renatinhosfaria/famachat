...
export class PgStorage implements IStorage {
  ...
  constructor() {
    ...
    this.userCache = new Cache<User>(5 * 60 * 1000); // 5 minutos (reduzido de 10 minutos)
    ...
    this.leadsCache = new Cache<Lead[]>(60 * 1000); // 1 minuto para cache de leads
  ...
  async createLead(insertLead: InsertLead): Promise<Lead> {
    try {
      ...
      const leadWithTimestamps = {
        ...insertLead,
        // Definir status padrão como "Sem Atendimento" se não for especificado
        status: insertLead.status || LeadStatusEnum.SEM_ATENDIMENTO,
        createdAt: now,
        updatedAt: now
      };
      ...
      const lead = await this.getLead(result[0].id);
      if (!lead) {
        throw new Error("Erro ao buscar lead após criação");
      }
  ...
  async getProductionReport(period?: string): Promise<ProductionReportResult> {
    try {
      ...
      for (const user of users) {
        if (user.department === schema.Department.SALES) {
          byUser[user.id] = {
            userId: user.id,
  ...
  async syncLeadStatus(clienteId: number, status: string, direction: 'cliente_to_lead' | 'lead_to_cliente' = 'cliente_to_lead'): Promise<boolean> {
    try {
      ...
      const statusMapping: Record<string, string> = {
        // Cliente para Lead
        'Sem Atendimento': LeadStatusEnum.SEM_ATENDIMENTO,
        'Em Atendimento': LeadStatusEnum.EM_ATENDIMENTO,
        'QUALIFICADO': LeadStatusEnum.QUALIFICADO,
        'Qualificado': LeadStatusEnum.QUALIFICADO,
        'Novo Lead': LeadStatusEnum.SEM_ATENDIMENTO,
        // Lead para Cliente
        [LeadStatusEnum.SEM_ATENDIMENTO]: ClienteStatus.SEM_ATENDIMENTO,
        [LeadStatusEnum.EM_ATENDIMENTO]: ClienteStatus.EM_ATENDIMENTO,
        [LeadStatusEnum.QUALIFICADO]: ClienteStatus.QUALIFICADO,
        [LeadStatusEnum.CONVERTIDO]: ClienteStatus.CONVERTIDO,
        [LeadStatusEnum.NAO_CONVERTIDO]: ClienteStatus.SEM_ATENDIMENTO,
      };
