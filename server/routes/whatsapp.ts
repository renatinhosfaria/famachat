...
export function registerWhatsappRoutes(app: Express) {
          ...
          if (apiConfig) {
            ...
            if (evolutionInstances.data && Array.isArray(evolutionInstances.data) && evolutionInstances.data.length > 0) {
              ...
              for (const apiInstance of evolutionInstances.data) {
                ...
                if (!existingInstance && instanceName) {
                  ...
                  await storage.createWhatsappInstance({
                    ...
                    instanceName,
                    status: apiInstance.connectionStatus === 'open' ? WhatsAppInstanceStatus.CONNECTED : WhatsAppInstanceStatus.DISCONNECTED,
                    userId: 1, // Admin
                    base64: null,
                    webhook: null
                  });
              ...
              return res.json(refreshedInstances.map(instance => ({
                ...instance,
                whatsappName: instance.status === WhatsAppInstanceStatus.CONNECTED ? "ConsultorAI" : "Desconectado",
                whatsappPhone: instance.status === WhatsAppInstanceStatus.CONNECTED ? "553499602714" : "Desconectado"
              })));
            }
          }
        } catch (apiError) {
  ...
  app.post("/api/whatsapp/instances", async (req, res) => {
    try {
      ...
      // Gerar instanciaId automaticamente se não for enviado
      if (!instanceData.instanciaId) {
        instanceData.instanciaId = `${instanceData.instanceName.replace(/[^a-zA-Z0-9]/g, '')}_${Date.now()}`;
      }
      
      // Verificar se o usuário já possui uma instância
      const existingInstance = await storage.getWhatsappInstanceByUser(instanceData.userId);
      if (existingInstance) {
      ...
      const user = await storage.getUser(instanceData.userId);
      if (!user) {
      ...
      try {
        ...
        logger.info(`Instância ${instanceData.instanceName} criada com sucesso na Evolution API`);
        ...
        logger.debug(`Gerando ID único para instância: ${uniqueInstanceId}`);
        
        // Criar a instância no nosso banco de dados com os campos adicionais
        const instance = await storage.createWhatsappInstance({
          ...instanceData,
          // Definir o instancia_id manualmente já que é um campo TEXT no banco
          instanciaId: uniqueInstanceId,
          // Usar o status da resposta da API ou o default DISCONNECTED
          status: response.data && response.data.instance && response.data.instance.status 
            ? response.data.instance.status 
            : WhatsAppInstanceStatus.DISCONNECTED,
          // Adicionar o campo base64 para o QR code
          base64: base64
          // Removido isPrimary que não existe mais no schema
        });
  ...
  app.get("/api/whatsapp/qrcode/:instanceName", async (req: Request, res: Response) => {
    try {
      ...
      if (instance.status && instance.status === WhatsAppInstanceStatus.CONNECTED) {
    ...
    try {
      ...
      if (lastCheck && now - lastCheck < MIN_CHECK_INTERVAL) {
        ...
        return res.json({
          ...
          status: instanceToUse.status === WhatsAppInstanceStatus.CONNECTED ? "CONNECTED" : "DISCONNECTED",
      ...
      if (instanceToUse.status !== newDbStatus) {
        logger.info(`Atualizando status da instância ${instanceToUse.instanceName} de ${instanceToUse.status} para ${newDbStatus}`);
  ...
  app.post("/api/whatsapp/verify-actual-status/:instanceId", async (req, res) => {
    try {
      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          for (const item of response.data) {
            if (item.instance && item.instance.instanceName === instance.instanceName) {
              instanceFound = true;
              connectionStatus = item.instance.state || item.instance.status || item.instance.connectionStatus;
        ...
        if (instance.status !== newDbStatus) {
          logger.info(`Atualizando status da instância ${instance.instanceName} de ${instance.status} para ${newDbStatus} com base na verificação direta`);
          ...
          logger.info(`Status da instância ${instance.instanceName} mantido como ${instance.status} (valor da API: ${connectionStatus})`);
        }
        ...
        return res.json({
          ...
          newStatus: newDbStatus,
          updated: instance.status !== newDbStatus
        });
  ...
  const statusCheckCache = new Map<number, {
    data: any,
    timestamp: number,
    expiresAt: number
  }>();
  ...
  const statusHistory = new Map<number, {
          ...
          if (instance) {
            ...
            if (apiUrl && apiKey) {
              try {
                  ...
                  if (extractedState) {
                    ...
                    if (!statusHistory.has(instanceId)) {
                      statusHistory.set(instanceId, {
                        ...
                        lastStableStatus: instance.status || 'DISCONNECTED'
                      });
                    }
                    ...
                    if (apiStatus === 'OPEN' || apiStatus === 'CONNECTED') {
                      ...
                      const isConnecting = instance.status === WhatsAppInstanceStatus.WAITING_QR_SCAN || 
                      ...
                      if ((instance.status === WhatsAppInstanceStatus.CONNECTED && history.consecutiveCloseCount < 3) ||
                          (isConnecting && history.consecutiveCloseCount < 2)) {
                        logger.warn(`Recebido status CLOSE, mas ignorando por enquanto (${history.consecutiveCloseCount}/3) - Status atual: ${instance.status}`);
                      ...
                      else if ((instance.status === WhatsAppInstanceStatus.CONNECTED && history.consecutiveCloseCount >= 3) ||
                               (isConnecting && history.consecutiveCloseCount >= 2) ||
                               (!isConnecting && instance.status !== WhatsAppInstanceStatus.CONNECTED)) {
                    ...
                    if (apiStatus === 'CONNECTING') {
                      ...
                      else if (instance.status && instance.status === WhatsAppInstanceStatus.CONNECTING) {
                ...
                if (instance.status !== WhatsAppInstanceStatus[newStatus]) {
                  logger.info(`Atualizando status da instância ${instance.instanceName} de ${instance.status} para ${WhatsAppInstanceStatus[newStatus]}`);
  ...
  app.get("/api/whatsapp/check-numbers", async (req, res) => {
    try {
      ...
      if (primaryInstance.status !== WhatsAppInstanceStatus.CONNECTED) {
        logger.warn(`Instância ${primaryInstance.instanceName} não está marcada como conectada no banco de dados (status: ${primaryInstance.status})`);
  ...
  app.get("/api/whatsapp/check-all-numbers", async (req, res) => {
    try {
      ...
      const connectedInstances = instances.filter(
        inst => inst.status === WhatsAppInstanceStatus.CONNECTED
      );
    ...
    try {
      ...
      if (primaryInstance.status !== WhatsAppInstanceStatus.CONNECTED) {
        return res.status(400).json({
          ...
          details: `O status atual é: ${primaryInstance.status}. Conecte a instância primeiro.`
        });
      }
    ...
    try {
      ...
      if (clienteIds && clienteIds.length > 0) {
        ...
        clientesSemJid = await db.query.clientes.findMany({
          where: (clientes, { and, eq, isNull, inArray }) => 
            and(
        ...
        try {
          ...
          if (updateMissingJids) {
            ...
            if (clienteIds && clienteIds.length > 0) {
              // Se recebermos uma lista de IDs, filtrar apenas esses clientes
              clientesSemJid = await db.query.clientes.findMany({
                where: (clientes, { and, eq, or, isNull, inArray }) => 
                  and(
                    eq(clientes.hasWhatsapp, true),
    ...
    try {
      ...
      if (primaryInstance.status !== WhatsAppInstanceStatus.CONNECTED) {
        return res.status(400).json({
          ...
          instanceStatus: primaryInstance.status
        });
      }
  ...
  app.post("/api/whatsapp/verify-actual-status/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (normalizedStatus !== instance.status) {
        ...
        updated = true;
        logger.info(`Status da instância atualizado de ${instance.status} para ${normalizedStatus}`);
      }
