...
router.get('/:id/contatos', async (req, res) => {
  try {
    ...
    const result = await db.execute(sql`
      SELECT cc.nome, cc.telefone, cc.email
      FROM imoveis_empreendimentos e 
      LEFT JOIN imoveis_contatos_construtora cc ON e.id_construtora = cc.id_construtora 
      WHERE e.id_empreendimento = ${parseInt(id)}
    `);

    res.json(result.rows);
  } catch (error) {
    logger.error('Erro ao buscar contatos da construtora:', error);
    res.status(500).json({ 
      error: 'Erro ao buscar contatos da construtora',
